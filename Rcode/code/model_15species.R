...
saveRDS(model,file ="model_15sp_sjsdm_12042023.RDS")
saveRDS(an,file ="model_an_15sp_sjsdm_12042023.RDS")

R2 = Rsquared(model)
summary.p<-summary(model)
co.env.spe <- cov2cor(getCov(model))
save(R2,summary.p,co.env.spe,file = "model_15sp_result_12042023.rdata")
...
split = sklearn$model_selection$IterativeStratification(n_splits = 20L,order = 2L) #(env, Y, test_size = 0.5,)
...
save(train_aucs, test_aucs ,file = "model_15sp_AUC_12042023.rdata")